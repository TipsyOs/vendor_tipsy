...
func init() {
	android.RegisterModuleType("tipsy_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = tipsyExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = tipsyExpandVariables(ctx, dep_file)
	...
	cmd := tipsyExpandVariables(ctx, String(g.properties.Cmd))
